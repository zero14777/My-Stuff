...
int main ( int argc, char* argv [])
{
	...
	if (libraryBooks == NULL || searchBooks == NULL)
	{
		fprintf (stderr, "One or more of the given files does not exist\n");
	...
	if (dictSize <= 0)
	{
		fprintf (stderr, "Improperly formatted Library Data File\n");
		fclose (libraryBooks);
		fclose (searchBooks);
		return 1;
	}
	...
	for (i = 0; i < numSearches; i++)
	{
		...
		if (buffer[j] == EOF || buffer[j] == '\0')
		{
			fprintf (stderr, "More search requests than book names given\n");
			break;
		}
