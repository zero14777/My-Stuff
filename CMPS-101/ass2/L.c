 ...
 * the list, and a long integer data value. Each list has pointers
...
void InsertionSort (ListHndl list, long data)
{
	...
	while (!AtLast (list) && list->current->data < data)
	{
		MoveNext (list);
	}

	if (AtLast (list) && list->current->data < data)
	{
...
void PrintCurrent (FILE* out, ListHndl list)
{
	...
	fprintf (out, "%lu", list->current->data);
}
