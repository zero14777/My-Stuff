# Creators: Jim Brower & Peter Greer-Berezovsky

# This code implements a simple distributed key-value store
# that is designed to run on 5 nodes and is built to have
# a focus on availability and partition tolerance.
...
def kvs(key):
	#Check to make sure the client made a legal request
    if re.search("[^a-zA-Z0-9_]", key) or len (key) > 250:
		...
		# Forward the put request to your leader and return whatever
		# your leader gives back to you.
		# Timeout if the leader doesn't respond and return an error.
		# If you are your own leader try putting the key into your
		# own table and return success, or replaced.
        ...
        # Return a the requested value if possible,
		...
		# Forward the delete request to your leader and return whatever
		# your leader gives back to you.
		# Timeout if the leader doesn't respond and return an error.
		# If you are your own leader try deleting the key from your
		# own table and return success, or error if the key didn't exist.
...
# This function is called every 2 seconds and maintains consistency across nodes
...
def heartbeat():
    ...
    # Check all nodes you can connect to and choose the one with
	# the highest rank to be your leader.
